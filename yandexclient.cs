...
namespace Yandex
{
    public class YandexMusicClient
    {
        ...
        public List<YTrackDownloadInfo> GetDownInfo(string idd)
        {
            var res = api.Track.GetMetadataForDownload(storage, idd).Result;
            return res; 
        }
        public YStorageDownloadFile TrackDownload(YTrackDownloadInfo info)
        {
            var res = api.Track.GetDownloadFileInfo(storage, info);
            return res;
        }
        public string BuildUrl( YTrack track)
        {
            var res = api.Track.GetFileLink(storage, track);
            return res;
        }
        #endregion Треки
        ...
        public YPlaylist GetPlaylist(string user, string kind)
        {
            return api.Playlist.Get(storage, user, kind).Result;
        }
        ...
        public YPlaylist ChangePlaylistTracks(YPlaylist pl, YTrack[] tracksOld, YTrack[] tracksNew)
        {
            //api.Playlist.
            var resp1 = api.Playlist.DeleteTracks(storage, pl, tracksOld);
            var plC = resp1.Result;
            var resp2 = api.Playlist.InsertTracks(storage, pl, tracksNew);
            return resp2.Result;
        }

        public YPlaylist CreateWithTracks(string name, YTrack[] tracksNew)
        {
            var res1 = api.Playlist.Create(storage, name).Result;
            var res2 = api.Playlist.InsertTracks(storage, res1, tracksNew).Result;
            return res2;
        }
